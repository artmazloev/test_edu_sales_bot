...
| LLM | GPT-4o |
| STT | OpenAI Whisper |
| TTS | OpenAI TTS HD (onyx / nova по сценарию) |
| Аудио | pydub + ffmpeg |
| Состояние | In-memory (dict по user_id) |

> Экономичная конфигурация (gpt-4o-mini + tts-1 + alloy) задокументирована в [MODELS.md](MODELS.md) для быстрого отката.
...
Инструкция по переключению на экономичный режим — в [MODELS.md](MODELS.md).
